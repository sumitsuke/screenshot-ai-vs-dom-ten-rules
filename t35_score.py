...
VISUAL = {"R2", "R4", "R5"}
METRIC = {"R1", "R3", "R6", "R7", "R8", "R9", "R10"}
CONTRAST = {"R1", "R2", "R3", "R4"}  # 対照あり＝gold に違反と非違反の両方が在るルール
...
def parse(text):
    ...
    for m in ANS.finditer(text):
        r, w, a = (
            "R" + m.group(1),
            m.group(2),
            m.group(3).upper() if m.group(3).upper() in ("YES", "NO") else m.group(3),
        )
        out[(r, w)] = a
    return out
...
def gold_pairs(page):
    for r, x in G[page].items():
        if isinstance(x["label"], dict):
            for w, l in x["label"].items():
                yield (r, w, l)
        else:
            yield (r, None, x["label"])


def bucket(gold, ans):
    if gold == "該当なし":
        return "除外" if ans == "該当なし" else ("無い要素に答えた" if ans in ("YES", "NO") else "除外")
    if ans is None:
        return "未回答"
    if ans == "不明":
        return "不明"
...
models = sorted(
    {
        ...
        for f in glob.glob(os.path.join(HERE, "responses", "*_*.txt"))
        if not os.path.basename(f).startswith("_") and not os.path.basename(f).startswith("pilot")
    }
)
lines = [
...
for model in models:
    ...
    for page, slug in SLUG.items():
        ...
        for r, w, gold in gold_pairs(page):
            ...
            if a is None and w:
                a = ans.get((r, None))  # 幅を書かずに 1 つ答えた場合は両幅に当てる
            if a is None and not w:
                a = ans.get((r, "375")) or ans.get((r, "1280"))
            rows.append(
                {
                    "model": model,
                    "page": page,
                    ...
                    "gold": gold,
                    "answer": a or "",
                    "bucket": bucket(gold, a),
                    "group": "目視" if r in VISUAL else "計測",
                }
            )
    with io.open(os.path.join(HERE, f"score_{model}.csv"), "w", encoding="utf-8", newline="") as fh:
        ...
        for x in rows:
            fh.write(
                ",".join(str(x[k]) for k in ("model", "page", "rule", "width", "gold", "answer", "bucket", "group"))
                + "\n"
            )
    for grp in (
        "全体",
        ...
        if grp.startswith("対照あり"):
            rs = [
                x
                for x in rows
                if x["rule"] in CONTRAST
                and (grp == "対照あり R1〜R4" or x["group"] == grp.split("・")[1].split(" ")[0])
            ]
        elif grp.startswith("対照なし"):
            rs = [x for x in rows if x["rule"] not in CONTRAST]
        else:
            rs = [x for x in rows if grp == "全体" or x["group"] == grp]
        c = Counter(x["bucket"] for x in rs)
        n = sum(c[k] for k in ("TP", "FP", "FN", "TN", "不明", "取り違え", "未回答"))
        dec = c["TP"] + c["FP"] + c["FN"] + c["TN"]
        f = lambda a, b: f"{a / b * 100:.0f}%" if b else "—"
        lines.append(
            f"| {model} | {grp} | {n}（頁 {len({x['page'] for x in rs})}） | {f(dec, n)} | {f(c['TP'] + c['TN'], dec)} | {f(c['不明'], n)} | {f(c['FP'], c['FP'] + c['TN'])} | {f(c['FN'], c['TP'] + c['FN'])} | {c['取り違え']} | {c['無い要素に答えた']} | {c['未回答']} |"
        )
    per_rule = defaultdict(Counter)
    for x in rows:
        per_rule[x["rule"]][x["bucket"]] += 1
    lines.append(
        f"| {model} | ルール別 | "
        + " / ".join(
            f"{r}: " + ",".join(f"{k}{v}" for k, v in sorted(per_rule[r].items()))
            for r in sorted(per_rule, key=lambda s: int(s[1:]))
        )
        + " | | | | | | | | |"
    )
