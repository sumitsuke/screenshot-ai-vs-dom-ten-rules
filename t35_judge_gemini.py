...
import io, os, sys, json, base64, time, winreg, urllib.request
...
def key():
    ...
    if not v:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment") as k:
            v = winreg.QueryValueEx(k, "GEMINI_API_KEY")[0]
    return v
...
OUT = os.path.join(HERE, "responses")
...
for slug in sorted(os.listdir(SHOTS)):
        ...
        parts.append(
            {
                "inline_data": {
                    "mime_type": "image/png",
                    "data": base64.b64encode(open(os.path.join(d, f), "rb").read()).decode(),
                }
            }
        )
    ...
    io.open(os.path.join(OUT, f"Gemini_{slug}.json"), "w", encoding="utf-8").write(
        json.dumps(
            {
                "model": MODEL,
